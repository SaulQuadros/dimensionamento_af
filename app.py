...
st.set_page_config(page_title='SPAF – Form + Matriz L_eq (Compatível PVC/FoFo)', layout='wide')
st.title('Dimensionamento – Barrilete e Colunas (Compatível com Quadros PVC/FoFo)')
...
# Identify diameter column in a table (PVC/FoFo)
...
def piece_columns_for(table):
    ...
    # exclude the diameter and any reference column
    cols = [c for c in table.columns if c not in (dn_name, 'dref_pol')]
    return cols, dn_name

# Lookup nearest row by user dn_mm and return dict + (de_ref_mm, pol_ref)
...
tab1, tab2, tab3 = st.tabs(['1) Trechos (Form)','2) Matriz de L_eq','3) Resultados & Exportar'])
...
with tab1:
    ...
    if ok:
        ...
        # pega a tabela do material para buscar os diâmetros de referência
        table_mat = pvc_table if material_sistema=='PVC' else fofo_table
        row_dict, de_ref_mm, pol_ref = lookup_row(table_mat, dn_mm)
        ...
        st.success('Trecho adicionado (diâmetro de referência preenchido automaticamente).')
...
# TAB 2 – Matriz de L_eq usando as colunas do material selecionado
with tab2:
    st.subheader('Matriz de Comprimentos Equivalentes (compatível com quadros do material)')
    ...
    if base.empty:
        ...
        dn_series, _ = get_dn_series(table_mat)

        tre = base.copy()
        tre['label'] = tre.apply(trecho_label, axis=1)

        # labels "bonitos" a partir dos nomes de coluna do CSV
        def pretty(col):
            lbl = col
            if lbl.endswith('_m'):
                lbl = lbl[:-2]
            ...
            # ajustes finos de abreviações
            lbl = lbl.replace('Te', 'Tê').replace('Angulo', 'Ângulo').replace('Pe', 'Pé')
            return lbl

        idx_labels = [pretty(c) for c in piece_cols]
        ...
        for _, r in tre.iterrows():
            ...
            m_vals = [ _num(eql.get(c, 0.0), 0.0) for c in piece_cols ]
            mat[f'{label} (m)']   = m_vals
            mat[f'{label} (Qt.)'] = [0]*len(m_vals)
            mat[f'{label} (Total)'] = [0.0]*len(m_vals)
        ...
        if ok2:
            ...
            for lab in tre['label']:
                total = 0.0
                for col, disp in zip(piece_cols, idx_labels):
            ...
            st.success('L_eq aplicado aos trechos.')
